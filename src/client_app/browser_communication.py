# browser_communication.py
from flask import Flask, request,jsonify
import global_variables
import threading
from flask_cors import CORS 
from crypto_utils import  encrypt_with_rsa_public_key;
import hashlib
import requests
import socket
import time
...
def receive_domain():
    ...
    global_variables.final_hash = ''

    domain_hash = hashlib.sha256(domain.encode('utf-8')).hexdigest().upper()
    password_hash = hashlib.sha256(password.encode('utf-8')).digest()

    global_variables.domain = domain_hash

    print(f"[도메인 수신됨] {domain}")
    print(f"[도메인 해시됨] {global_variables.domain}")
    print(f"[비밀번호 수신됨] {password}")
    print(f"[비밀번호 해시됨] {password_hash.hex().upper()}")

    random_bytes = bytes.fromhex(global_variables.random);
    encrypted_bytes = bytes.fromhex(global_variables.encrypted)
    domain_bytes = global_variables.domain.encode('utf-8')
    data_to_encrypt = random_bytes+encrypted_bytes+domain_bytes
    encrypted_result = encrypt_with_rsa_public_key(data_to_encrypt);
    print(f"현재 도메인 : {global_variables.domain.encode('utf-8')}")
    
    encrypted_bin_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "secure", "encrypted.bin")
    ...
    if (global_variables.running_trigger):
        try:
            ...
            while waited < timeout:
                if global_variables.final_hash != '':
                    break
                time.sleep(interval)
                waited += interval
            ...
            if global_variables.final_hash == "":
                ...
                print("[Flask] 최종 해시:", global_variables.final_hash)
                return jsonify({"status": "ok", "final_hash": global_variables.final_hash})
...
def receive_domain_thread():
    if global_variables.domain_thread_started:
        return
    global_variables.domain_thread_started = True

    thread = threading.Thread(target=start_server)
    thread.daemon=True
    thread.start()
def receive_final_hash_thread():
    if global_variables.final_hash_thread_started:
        return
    global_variables.final_hash_thread_started = True
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
            ...
            while True:
                ...
                with conn:
                    ...
                    global_variables.final_hash = final_hash.hex().upper()
                    print("[사용자 프로그램] 최종 해시 수신:", global_variables.final_hash)
